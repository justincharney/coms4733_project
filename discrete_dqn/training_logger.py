"""
Training logger for DQN training sessions.
Adapted from SAC_Agent/training_logger.py for DQN-specific metrics.
"""
...
class TrainingLogger:
    ...
    def log_episode(self, episode, reward, success_rate, loss_info=None, epsilon=None):
        """
        ...
        if self.log_to_file and self.csv_writer:
            row = {
                ...
                "episode_length": 1,  # DQN baseline: 1 step per episode (contextual bandit)
                "loss": loss_info.get("loss", 0.0) if loss_info else 0.0,
                "q_mean": loss_info.get("q_mean", 0.0) if loss_info else 0.0,
                "q_std": loss_info.get("q_std", 0.0) if loss_info else 0.0,
